...
/**
 * Static 2017 batch student detail — same layout as other batch profiles.
 */
export default function Batch2017StudentProfile() {
  const { slug } = useParams();
  const student = slug ? getBatch2017BySlug(slug) : null;
  const [lightbox, setLightbox] = useState(null);

  if (!student) {
    return <Navigate to="/batch-2017-students" replace />;
  }
  ...
  return (
    <div className="alumni-profile-page">
      <div className="alumni-profile-inner">
        <Link to="/batch-2017-students" className="alumni-profile-back">
          ← 2017 batch students
        </Link>

        <article className="alumni-profile-card">
          <p className="alumni-profile-eyebrow">2017 batch</p>
